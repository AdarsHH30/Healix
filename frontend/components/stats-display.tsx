...
function StatCard({ value, label, color, delay = 0 }: StatCardProps) {
  ...
  return (
    ...
    >
      <div className={`text-3xl font-bold ${colorClasses[color].split(" ")[0]} mb-2`}>{value}</div>
...
export function StatsDisplay() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full max-w-6xl">
      <StatCard value="24/7" label="Access Anytime" color="primary" delay={0} />
      <StatCard value="Expert" label="Certified Instructors" color="accent" delay={75} />
      <StatCard value="Custom" label="Personalized Plans" color="secondary" delay={150} />
      <StatCard value="Community" label="Supportive Network" color="primary" delay={225} />
    </div>
  )
}
